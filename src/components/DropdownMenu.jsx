    ...
    <li className='relative group cursor-pointer'>
      ...
      <ul className='absolute left-0 w-48 bg-white shadow-md rounded-md hidden group-hover:block'>
        {courses.map((item) => (
          <li key={item.id}>
            <NavLink
              to={`/CourseDetailPage?id=${item.id}`} // Ensure the correct route
              onClick={() => handleCourseDetailClick(item.id)}
              className={getActiveClass(item.id)} // Use dynamic active class
            >
              {item.title}
            </NavLink>
          </li>
